...
def strategy_fuzzy(asset, df):
    # 数据进策略
    n1, n2, aa = fuzzy(df)
    # 计算结果最后一位没值，所以拿最后5位有值的，再取个平均数
    mood = aa[1, 0, n2 - 6:n2 - 1] - aa[0, 0, n2 - 6:n2 - 1]  # a7-a6的值，正：可以买
    avmood = np.mean(mood)

    # 若空仓，且大买家占优，则买
    if 0 == len(asset.positionEntity.currentOrders) and avmood > 0:  # 空仓时买
        # 构建交易记录
        trade_point = [asset.tick_time.strftime('%Y-%m-%d %H:%M:%S'),
                       round(asset.tick_close, 3),
                       "buy"]
        # 将交易记录填入仓位
        asset.positionEntity.trade_point_list.append(trade_point)
        volume = int(asset.positionEntity.money / asset.tick_close / 100)
        # 全仓买，除以100算出能买多少手
        PSLAsset.buy(asset, volume)
    # 满仓
    if 0 != len(asset.positionEntity.currentOrders) and avmood < 0:
        trade_point = [asset.tick_time.strftime('%Y-%m-%d %H:%M:%S'),
                       round(asset.tick_close, 3),
                       "sell"]
        asset.positionEntity.trade_point_list.append(trade_point)
...
if __name__ == '__main__':
    ...
    directory_path = PSLTools.read_config("PSLData", "live_bar")
    char_to_find = '_d'
